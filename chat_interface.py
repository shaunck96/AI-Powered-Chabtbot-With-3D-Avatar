...
def summary_generation(docs, collection_name='AlphaDataFinal'):
    ...
    prompt_checker = f"""Write a concise summary of the following describing keywords and key points discussed:
    "{docs.page_content}"
    CONCISE SUMMARY:"""
    prompt_length = num_tokens_from_string(prompt_checker, "cl100k_base")
    ...
    if prompt_length<16000:
        ...
        final_summary = []
        texts = text_splitter.create_documents([docs.page_content])
        for text in texts:
            final_summary.append(chain.run([text]))
...
class Chatbot():
    def __init__(self):
        ...
        load_preprocess_cluster(r'sample_data\whisper (1).pdf')
